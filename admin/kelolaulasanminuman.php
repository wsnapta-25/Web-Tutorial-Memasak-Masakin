...
if (isset($_GET['aksi']) && $_GET['aksi'] == 'hapus' && isset($_GET['data'])) {
    ...
    $stmt->execute();
    $stmt->close();
    header("Location: kelolaulasanminuman.php?notif=hapusberhasil");
    exit;
}

$id_pengguna = $_SESSION['id_pengguna'];
$sql = "select `nama`, `email_pengguna`, `kontak`, `id_ulasan`, `ulasan`, `nama_resep` from `pengguna` inner join `ulasan` on `pengguna`.`id_pengguna` = `ulasan`.`id_pengguna` inner join `info_resep` on `ulasan`.`id_info_resep` = `info_resep`.`id_info_resep` where `ulasan`.`id_jenis` = 2";
    ...
    <?php if (empty($feedbacks)) { ?>
        <div class="card-kelola">
            <div class="bio-feedback">
                <div style="text-align: center;">Tidak ada ulasan pada minuman.</div>
            </div>
        </div>
    <?php } else { ?>
